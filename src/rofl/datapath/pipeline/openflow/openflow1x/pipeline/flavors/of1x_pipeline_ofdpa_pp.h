...
#ifndef __OF1X_PIPELINE_OFDPA_PP_H__
#define __OF1X_PIPELINE_OFDPA_PP_H__
...
static inline void __of1x_process_packet_pipeline_ofdpa(const unsigned int tid, const of_switch_t *sw, datapacket_t *const pkt){
	...
	do {
		...
		ROFL_PIPELINE_INFO("Packet[%p] doing match lookup at table: %u\n", pkt, table->number);
		...
		if(likely(match != NULL)){
			...
			if((table_to_go > table->number) && likely(table_to_go < OF1X_MAX_FLOWTABLES) && bitmap256_is_bit_set(&(table->config.goto_tables), table_to_go)){
				...
				//Get table index
				for (j = 0; j < ((of1x_switch_t*)sw)->pipeline.num_of_tables; ++j) {
					if (((of1x_switch_t*)sw)->pipeline.tables[j].number == table_to_go) {
						i = ((of1x_switch_t*)sw)->pipeline.tables[j].table_index;
						ROFL_PIPELINE_INFO("Packet[%p] Going to table %u->%u (table-index: %u)\n",pkt, table->number, table_to_go, i);
						break;
					}
				}
...
#endif
				ROFL_PIPELINE_INFO("Packet[%p] continuing in pipeline\n", pkt);

				continue;
			}

			ROFL_PIPELINE_INFO("Packet[%p] applying write actions\n",pkt);
			...
			__of1x_process_write_actions(tid, (of1x_switch_t*)sw, table->number, pkt, /*enforce packet cloning*/true);
			...
			if(table->default_action == OF1X_TABLE_MISS_DROP){
				...
				platform_of1x_packet_in((of1x_switch_t*)sw, table->number, pkt, ((of1x_switch_t*)sw)->pipeline.miss_send_len, OF1X_PKT_IN_NO_MATCH);
				return;
			}
			//else -> continue with the pipeline
			i = table->table_index_next;
		}

	} while (i > 0); //continue with next table

	ROFL_PIPELINE_INFO("Dropping packet[%p] in switch %s [%p] pipeline (1.X)\n",pkt,sw->name, sw);
...
* @ingroup core_pp 
*/
static inline void __of1x_process_packet_out_pipeline_ofdpa(const unsigned int tid, const of1x_switch_t *sw, datapacket_t *const pkt, const of1x_action_group_t* apply_actions_group){
	...
	if(reinject_pkt)
		__of1x_process_packet_pipeline_ofdpa(ROFL_PIPELINE_LOCKED_TID, (const of_switch_t*)sw, reinject_pkt);
