...
void of1x_dump_table(of1x_flow_table_t* table, bool raw_nbo){
	...
	ROFL_PIPELINE_INFO("Dumping table # %s(%u) (%p). Default action: %s, table-id: %u, next table-id: %u, num. of entries: %d, ma: %u statistics {looked up: %u, matched: %u}\n", table->name, table->number, table, __of1x_flow_table_miss_config_str[table->default_action], table->table_index, table->table_index_next, table->num_of_entries, table->matching_algorithm,  c.lookup_count, c.matched_count);
